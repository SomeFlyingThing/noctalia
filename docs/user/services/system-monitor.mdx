...
cpu_poll_seconds = 2.0              # CPU usage, temperature, load average, and on-demand frequency
...
Poll intervals are configured globally under `[system.monitor]`, not per widget. Each value is the number of seconds between samples for that metric group. A non-zero value is clamped to `1` to `120` seconds; setting a value to `0` disables that metric group entirely - it is never sampled and schedules no wakeups. CPU frequency is sampled at `cpu_poll_seconds` only while a CPU-frequency widget, a `systemStats()` plugin, or a visible `sysmon` tooltip needs it. GPU probes likewise run only while a widget, plugin, or other consumer needs them, so their default intervals do not wake otherwise unused hardware. Set the corresponding poll interval to `0` to disable a metric group entirely. You can also adjust intervals in whole-second steps (down to `0`) in **Settings → Services → Advanced** when the system monitor is enabled.

| Key | Default | Metrics |
|-----|---------|---------|
| `cpu_poll_seconds` | `2.0` | CPU usage %, CPU temperature, load average (`1` / `5` / `15` min), and CPU frequency while it has a consumer |
